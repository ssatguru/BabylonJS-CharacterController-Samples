...
function main() {
  ...
  light.intensity = 0.3;
  var light2 = new DirectionalLight("light2", new Vector3(-1, -1, -1), scene);
  light2.position = new Vector3(0, 128, 0);
  light2.intensity = 0.7;
  var ground = createGround(scene);
  var steps = MeshBuilder.CreateBox("Steps", { width: 5, height: 0.25, depth: 5 }, scene);
...
function loadPlayer(scene, engine, canvas) {
  BABYLON.SceneLoader.ImportMesh("", "assets/player/", "Vincent.babylon", scene, function (meshes, particleSystems, skeletons) {
    var player = meshes[0];
    ...
    var cc = new CharacterController(player, camera, scene, null, true);
...
function createGroundMaterial(scene) {
  ...
  var diffuseTexture = new Texture("assets/ground/ground.jpg", scene);
  diffuseTexture.uScale = 4.0;
  diffuseTexture.vScale = 4.0;
  groundMaterial.diffuseTexture = diffuseTexture;
  var bumpTexture = new Texture("assets/ground/ground-normal.png", scene);
